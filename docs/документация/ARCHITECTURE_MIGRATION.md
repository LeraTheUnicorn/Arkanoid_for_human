...
- `game/game_config.py` - все константы игры
- `game/game_models.py` - модели данных (Ball, Paddle, GameState)
- `game/game_views.py` - представления для отрисовки
...
- `game/PyGameBall.py` - обновлен для использования констант из `game_config.py`
...
```python
from game.di_container import DIContainer
...
from game.game_models import Ball, GameState, Paddle
...
from game.game_views import BricksView, BallView, PaddleView, HUDView
...
Текущий код в `game/PyGameBall.py` продолжает работать, но теперь использует константы из `game/game_config.py`.
...
Старый код продолжает работать. Все константы доступны через импорт из `game.game_config`.
